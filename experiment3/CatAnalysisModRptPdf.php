...
$pdf->SetTitle('Experiment:Trip Generation : Category Analysis');
...
$pdf->Write(0, 'Experiment No. 3 ', '', 0, 'L', true, 0, false, false, 0);
$pdf->Write(0, 'Trip Generation: Category Analysis', '', 0, 'C', true, 0, false, false, 0);
...
if($file_ext1 == '.xls' )
{
	...
	// Trip File
        ...
        $pdf->Cell(($nCol*25),10,' Observed Socio-Economic Trip Data ',1,0,'C');
        for ($i = 1; $i <= $nRow; $i++)
        {    
        	$pdf->Ln(); //new row          
            for ($j = 1; $j <= $nCol; $j++)
...
switch($m_no_of_criteria)
{
		case 1: for($i=0;$i<=$m_no_of_groups[0];$i++)
				...
				$pdf->Cell(115,10,' Observed Socio-Economic Trip Data ',1,0,'C');
     			$pdf->Ln(); //new row
				...
				$pdf->Cell(25,10,'  No. Of Trips ',1,0,'C');
				$pdf->Cell(40,10,' Trip rate ',1,0,'C');
				$pdf->Ln(); //new row
				for($i=1;$i<=$m_no_of_groups[0];$i++)
				{
						...
						$pdf->Cell(25,10,$NoOfTrip[$i],1,0,'C');
						if($NoOfHouseHold[$i] != 0)
						{
							$pdf->Cell(40,10,($NoOfTrip[$i]/$NoOfHouseHold[$i]),1,0,'C');
						}
						else 
						{
							$pdf->Cell(40,10,'No Data Available ',1,0,'C');
						}
						if($NoOfHouseHold[$i] != 0)
						{
		...
		case 2:	for($i=0;$i<=$m_no_of_groups[0];$i++)
				...
				*/
				$pdf->Cell(140,10,' Observed Socio-Economic Trip Data ',1,0,'C');
     			$pdf->Ln(); //new row
				for($i=0;$i<$m_no_of_criteria;$i++)
				{
					$pdf->Cell(25,10,$m_CatMtx[1][$m_Category[$i]],1,0,'C');
				...
				for($i=1;$i<=$m_no_of_groups[0];$i++)
				{
					...
					for($j=1;$j<=$m_no_of_groups[1];$j++)
					{
						...
						$pdf->Cell(25,10,$NoOfTrip[$i][$j],1,0,'C');						
						if($Cat[$i][$j] != 0)
						{
							$pdf->Cell(40,10,($NoOfTrip[$i][$j]/$Cat[$i][$j]),1,0,'C');
						}
						else 
						{
							$pdf->Cell(40,10,'No Data Available ',1,0,'C');
		...
		case 3:for($i=0;$i<=$m_no_of_groups[0];$i++)
			...
			//			echo "Group : ".($k)."<br>";
  			...
  			//		echo "<br>";
				...
				*/
				$pdf->Cell(165,10,' Observed Socio-Economic Trip Data ',1,0,'C');
				$pdf->Ln(); //new row
				for($i=0;$i<$m_no_of_criteria;$i++)
				{
					$pdf->Cell(25,10,$m_CatMtx[1][$m_Category[$i]],1,0,'C');
				...
				for($i=1;$i<=$m_no_of_groups[0];$i++)
				{
					...
					for($j=1;$j<=$m_no_of_groups[1];$j++)
					{
						for($k=1;$k<=$m_no_of_groups[2];$k++)
						{
							...
							$pdf->Cell(25,10,$NoOfTrip[$i][$j][$k],1,0,'C');
						
							if($Cat[$i][$j][$k] != 0)
							{
								$pdf->Cell(40,10,($NoOfTrip[$i][$j][$k]/$Cat[$i][$j][$k]),1,0,'C');
							}
							else 
							{
								$pdf->Cell(40,10,'No Data Available ',1,0,'C');
			...
			//			echo "Group : ".($k)."<br>";
  			...
  			//		echo "<br>";
				...
				*/
				...
				$pdf->Cell(190,10,' Observed Socio-Economic Trip Data ',1,0,'C');
     			$pdf->Ln(); //new row
				for($i=0;$i<$m_no_of_criteria;$i++)
				{
					$pdf->Cell(25,10,$m_CatMtx[1][$m_Category[$i]],1,0,'C');
					...
					for($j=1;$j<=$m_no_of_groups[1];$j++)
					{
						for($k=1;$k<=$m_no_of_groups[2];$k++)
						{
							for($l=1;$l<=$m_no_of_groups[3];$l++)
							{
								...
								$pdf->Cell(25,10,$NoOfTrip[$i][$j][$k][$l],1,0,'C');
								if($Cat[$i][$j][$k][$l] != 0)
								{
									$pdf->Cell(40,10,($NoOfTrip[$i][$j][$k][$l]/$Cat[$i][$j][$k][$l]),1,0,'C');
								}
								else 
								{
									$pdf->Cell(40,10,'No Data Available ',1,0,'C');
		...
		case 5:for($i=0;$i<=$m_no_of_groups[0];$i++)
				...
				*/
				...
				$pdf->Cell(215,10,' Observed Socio-Economic Trip Data ',1,0,'C');
     			$pdf->Ln(); //new row
				for($i=0;$i<$m_no_of_criteria;$i++)
				{
					$pdf->Cell(25,10,$m_CatMtx[1][$m_Category[$i]],1,0,'C');
						...
						for($k=1;$k<=$m_no_of_groups[2];$k++)
						{
							for($l=1;$l<=$m_no_of_groups[3];$l++)
							{
								for($p=1;$p<=$m_no_of_groups[4];$p++)
								{
									...
									$pdf->Cell(25,10,$NoOfTrip[$i][$j][$k][$l][$p],1,0,'C');
									if($Cat[$i][$j][$k][$l][$p] != 0)
									{
										$pdf->Cell(40,10,($NoOfTrip[$i][$j][$k][$l][$p]/$Cat[$i][$j][$k][$l][$p]),1,0,'C');
									}
									else 
									{
										$pdf->Cell(40,10,'No Data Available ',1,0,'C');
...
$pdf->Output($UploadFile.'/Experiment3/CategoryAnalysis.pdf',"F");
